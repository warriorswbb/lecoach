...
# Load the data
file_path = 'pbp.csv'  # Make sure to use the correct path
...
if filtered_data.empty:
    st.warning("No data available for the selected filters. Please choose a different season, player, or team.")
else:
    ...
    fig = px.density_contour(filtered_data, x='shot_x', y='shot_y', title=f"Shot Position Density Contour Plot for {selected_season} - {selected_team if selected_team != 'All Teams' else 'All Teams'} - {selected_player if selected_player != 'All Players' else 'All Players'}")
    ...
    fig.update_layout(width=800, height=800, xaxis_title="Shot X", yaxis_title="Shot Y")
